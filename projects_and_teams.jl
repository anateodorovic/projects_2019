
###############################################################################

projects = Project[
	Project(
		name             = "Legend Of Zelda",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = true,
			text = """
			""",
			resources = URL[
				URL(""),
				URL(""),
			],
			sequel_resources = Dict{String, Solution}(
				"Legend Of Zelda" => Solution(
					repo        = URL("https://github.com/CubeLord/LPRS2-Zelda.git"),
					youtube     = URL("https://www.youtube.com/watch?v=0CTFHpnBlLo&feature=youtu.be"),
					release_bit = URL("https://github.com/CubeLord/LPRS2-Zelda/blob/master/Legend_Of_Zelda.bit"),
					doc         = URL("https://github.com/CubeLord/LPRS2-Zelda/blob/master/legend_of_zelda.pdf"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Шах",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Nemanja, Mihajlo, Milan",
			members = Student[
				Student(
					name    = "Nemanja",
					surname = "Havlovic",
					index   = "RA-248/2014",
					email   = "nemanja.havlovic@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Mihajlo",
					surname = "Solda",
					index   = "RA-152/2016",
					email   = "mihajlosolda1997@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Milan",
					surname = "Pasic",
					index   = "RA-89/2016",
					email   = "mpasic9697@gmail.com",
					points  = 45,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/suncicam/MinesweeperProjekatLPRS2"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Racing Kings",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Miloš, Slobodan, Arsenije",
			members = Student[
				Student(
					name    = "Miloš",
					surname = "Ignjatović",
					index   = "RA-95/2016",
					email   = "ignjalignja0@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Slobodan",
					surname = "Kostić",
					index   = "RA-139/2016",
					email   = "slobodan.kostic14@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Arsenije",
					surname = "Govedarica",
					index   = "RA-215/2016",
					email   = "arsenije.govedarica@gmail.com",
					points  = 43,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/suncicam/MinesweeperProjekatLPRS2"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/skostic14/LPRS2_Racing_Kings"),
			youtube     = URL("https://www.youtube.com/watch?v=wm4ek4AhpNQ"),
			release_bit = URL("https://github.com/skostic14/LPRS2_Racing_Kings/blob/master/LPRS2_Racing_Kings.bit"),
			doc         = URL("https://github.com/skostic14/LPRS2_Racing_Kings/blob/master/LPRS2_Racing_Kings.docx"),
		),
	),
	Project(
		name             = "Advance Tetris",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Andrej, Jovana",
			members = Student[
				Student(
					name    = "Jovana",
					surname = "Crveni",
					index   = "RA-175/2016",
					email   = "jovana.crveni@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Andrej",
					surname = "Lazic",
					index   = "RA-246/2014",
					email   = "ane.lazic@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Filip",
					surname = "Mihic",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			- Gameplay dovršiti
			""",
			resources = URL[
				URL("https://github.com/boza94/pizeo_buzz"),
			],
			sequel_resources = Dict{String, Solution}(
				"Advance Tetris" => Solution(
					repo        = URL("https://github.com/Sandra2606996/MSAK_tetris"),
					youtube     = URL("https://www.youtube.com/watch?v=Q_IEFTvD00M"),
					release_bit = URL("https://github.com/Sandra2606996/MSAK_tetris/blob/master/tetris.bit"),
					doc         = URL("https://github.com/Sandra2606996/MSAK_tetris/blob/master/TETRIS.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Boomberman",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Nemanja, Aleksa, Saša",
			members = Student[
				Student(
					name    = "Nemanja",
					surname = "Lekić",
					index   = "RA-048/20155",
					email   = "nemanjalekic.ns@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Aleksa",
					surname = "Janjatović",
					index   = "RA-126/2016",
					email   = "janjatovic.aleksa@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Saša",
					surname = "Serafimovski",
					index   = "RA-169/2016",
					email   = "sasha97ns@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Boomberman" => Solution(
					repo        = URL("https://github.com/KakucaNebojsa/Boomberman"),
					youtube     = URL("https://www.youtube.com/watch?v=V5IfcWc_Ebo"),
					release_bit = URL("https://github.com/KakucaNebojsa/Boomberman/blob/master/Bomberman.bit"),
					doc         = URL("https://github.com/KakucaNebojsa/Boomberman/blob/master/Bomberman-dokumentacija.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "The World's Hardest Game",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Nikola, Miodrag, Laza",
			members = Student[
				Student(
					name    = "Laza",
					surname = "Jakovljevic",
					index   = "RA-205/2016",
					email   = "laza9977@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Miodrag",
					surname = "Pantic",
					index   = "RA-085/2016",
					email   = "miodrag.pantic97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Nikola",
					surname = "Tosic",
					index   = "RA-035/2016",
					email   = "nikola.tosic@live.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			- Više levela
			- Neki novi feature
			- Lepše
			""",
			resources = URL[
				URL("https://www.coolmathgames.com/0-worlds-hardest-game"),
				URL(""),
			],
			sequel_resources = Dict{String, Solution}(
				"The World's Hardest Game" => Solution(
					repo        = URL("https://github.com/AleksaArsic/LPRS2-WorldsHardestGame"),
					youtube     = URL("https://youtu.be/WKC8rEK4Ln4"),
					release_bit = URL("https://github.com/AleksaArsic/LPRS2-WorldsHardestGame/blob/master/LPRS2_Asteroids_game-master/WorldsHardestGame.bit"),
					doc         = URL("https://github.com/AleksaArsic/LPRS2-WorldsHardestGame/blob/master/WorldsHardestGame.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Heads foodball",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL(""),
				URL(""),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Catch the flowers",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Petar, Tamara, Miloš, Radovan",
			members = Student[
				Student(
					name    = "Petar",
					surname = "Zečević",
					index   = "RA-43/2016",
					email   = "pnz.vodka@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Tamara",
					surname = "Kuprešanin",
					index   = "RA-136/2016",
					email   = "tamarakupresanin@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Miloš",
					surname = "Poletanović",
					index   = "RA-93/2016",
					email   = "poletanovicmilos97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Radovan",
					surname = "Taušan",
					index   = "RA-184/2016",
					email   = "radovantausan67@gmail.com",
					points  = 0,
				)
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			- Padanje cveća i zemlje
			- Gameplay
			- Animacija zeke sa korpom
				- Okretanje korpe
				- Njihanje kad je korpa gore.
					- Zeka miran kad je nadole
			- Pozadina
				- Neka dodatna vrsta kompresije
			- Više vrsta cveća
			- Broj poena
			- Postepeno povećavati brzinu igre
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=duA4BPw3H6E"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/PetarZecevic/CatchTheFlowers"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Potapanje brodića",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Uroš, Vladimir, Srđan",
			members = Student[
				Student(
					name    = "Vladimir",
					surname = "Ostojic",
					index   = "RA-219/2015",
					email   = "vladimirostojic@outlook.com",
					points  = 0,
				),
				Student(
					name    = "Uros",
					surname = "Popovic",
					index   = "RA-194/2016",
					email   = "urospop97@gmail.com@",
					points  = 0,
				),
				Student(
					name    = "Srdjan",
					surname = "Bobos",
					index   = "RA-195/2016",
					email   = "srdjan.bobos@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Igor",
					surname = "Njagul",
					index   = "RA-115/2014",
					email   = "igor.njagul@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			- Dokumentovati:
				- Hardversko povezivanje ploča (slike, šema, opis)
				- Protokol puštanja aplikacija (prvo slave pa maste...)
			- Popraviti gameplay.
			- Uraditi napredniju grafiku:
				- Umesto text-mode grafike da bude
					9b color mode (Minesweeper) grafika
			""",
			resources = URL[
				URL("https://github.com/suncicam/MinesweeperProjekatLPRS2"),
			],
			sequel_resources = Dict{String, Solution}(
				"Potapanje brodića" => Solution(
					repo        = URL("https://github.com/uros96/potapanjeBrodica"),
					youtube     = URL("https://www.youtube.com/watch?v=carpYddvtfQ"),
					release_bit = URL[
						URL("https://github.com/uros96/potapanjeBrodica/blob/master/potapanje_brodica_master.bit"),
						URL("https://github.com/uros96/potapanjeBrodica/blob/master/potapanje_brodica_slave.bit"),
					],
					doc         = URL("https://github.com/uros96/potapanjeBrodica/blob/master/LPRS2-Potapanje_brodica.pdf"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Donkey Kong",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Nikolina, Mihajlo, Luka",
			members = Student[
				Student(
					name    = "Luka",
					surname = "Karan",
					index   = "RA-050/2016",
					email   = "lukakaran5@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Mihajlo",
					surname = "Milotic",
					index   = "RA-100/2016",
					email   = "mihajlo.milotic@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Nikolina",
					surname = "Bodiroga",
					index   = "RA-040/2016",
					email   = "nikolinabodiroga1@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/iamcarp/Donkey_Kong"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "T-Rex, Run!",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Andrija, Denis, Strahinja",
			members = Student[
				Student(
					name    = "Andrija",
					surname = "Jovanovic",
					index   = "RA-117/2016",
					email   = "akijo97@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Denis",
					surname = "Liliom",
					index   = "RA-031/2016",
					email   = "denis.liliom@gmail.com",
					points  = 45,
				),
				Student(
					name    = "Strahinja",
					surname = "Loncarski",
					index   = "RA-237/2016",
					email   = "strahinja.loncarski@gmail.com",
					points  = 44.5,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/LiliFakultet/TREX"),
			youtube     = URL("https://www.youtube.com/watch?v=Ske-izDszqQ"),
			release_bit = URL("https://github.com/LiliFakultet/TREX/blob/master/TREX.bit"),
			doc         = URL("https://github.com/LiliFakultet/TREX/tree/master/docs"),
		),
	),
	Project(
		name             = "Sensible Soccer",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Red Bouncing Ball Spikes",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Danilo, Darko, Bojan",
			members = Student[
				Student(
					name    = "Dainlo",
					surname = "Puskarevic",
					index   = "RA-225/2015",
					email   = "danilopuskarevic@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Darko",
					surname = "Rakic",
					index   = "RA-1/2016",
					email   = "rakicdarko00@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Bojan",
					surname = "Rikic",
					index   = "RA-134/2016",
					email   = "ler.marked@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=_vkTPFVyKoo"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Duck Hunt",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Aleksandar, Pavle, Miloš",
			members = Student[
				Student(
					name    = "Milos",
					surname = "Mozetic",
					index   = "RA-092/2016",
					email   = "mozeticmilos97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Pavle",
					surname = "Micovic",
					index   = "RA-207/2016",
					email   = "nspavle@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Aleksandar",
					surname = "Zagorac",
					index   = "RA-160/2016",
					email   = "aleksandarzagorac@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			- Dokumentovati:
				- Hardverski dizajn elektronike štoljpija (šema)
				- Hardversko povezivanje štoljpija (slike, šema, opis, pinovi)
				- Testiranje štoljpija sa Sega_Gun_Test projektom.
			- Dodati podršku za štoljpi u postojeću Duck Hunt igru.
			- Utegnuti gameplay.
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Duck Hunt" => Solution(
					repo        = URL("https://github.com/mladja777/duck_hunt"),
					youtube     = URL("https://www.youtube.com/watch?v=MpmgGTueH8o"),
					release_bit = URL("https://github.com/mladja777/duck_hunt/blob/master/battle_city_design/duck_hunt.bit"),
					doc         = URL("https://github.com/mladja777/duck_hunt/blob/master/Izvestaj.pdf"),
				),
				"Štoljpi za Duck Hunt" => Solution(
					repo        = URL("https://github.com/NikolaMal/Stoljpi_za_duck_hunt"),
					youtube     = URL("https://www.youtube.com/watch?v=vlJk6v707PM"),
					release_bit = URL("https://github.com/NikolaMal/Stoljpi_za_duck_hunt/blob/master/Stoljpi_za_duck_hunt/Stoljpi_Project/implementation/download.bit"),
					doc         = URL("https://github.com/NikolaMal/Stoljpi_za_duck_hunt/blob/master/stoljpi_doc.pdf"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Worms",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			Doraditi igru.
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Worms" => Solution(
					repo        = URL("https://github.com/BranislavNovak/Worms"),
					youtube     = URL("https://www.youtube.com/watch?v=Mi3ZPkKe-oc"),
					release_bit = URL("https://github.com/BranislavNovak/Worms/blob/master/worms.bit"),
					doc         = URL("https://github.com/BranislavNovak/Worms/blob/master/Worms.pdf"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "New Rally X",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			Doraditi igru.
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"New Rally X" => Solution(
					repo        = URL("https://github.com/0x8000/new_rally_x"),
					youtube     = URL("https://youtu.be/18Z-5beB13A"),
					release_bit = URL("https://github.com/0x8000/new_rally_x/blob/master/NewRallyX.bit"),
					doc         = URL("https://github.com/0x8000/new_rally_x/blob/master/Prezentacija%2C%20dokumentacija.pdf"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Risico",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Tap That Cat with Colors",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Pie with Rationals",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Tenkići",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Darko, Saša, Marko",
			members = Student[
				Student(
					name    = "Darko",
					surname = "Culum",
					index   = "RA-32/2016",
					email   = "cdarko97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Sasa",
					surname = "Bojanic",
					index   = "RA-179/2016",
					email   = "sasa-bojanic@hotmail.com",
					points  = 0,
				),
				Student(
					name    = "Marko",
					surname = "Travar",
					index   = "RA-203/2016",
					email   = "markotravar@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			Doraditi igru:
				- Animacije eksplozije
				- Bonusi
				- AI
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Tenkići" => Solution(
					repo        = URL("https://github.com/stefankoncarevic/Tenkici_ima_da_lete"),
					youtube     = URL("https://www.youtube.com/watch?v=Zlc82t1uIKM"),
					release_bit = URL("https://github.com/stefankoncarevic/Tenkici_ima_da_lete/blob/master/battle_city_design/demo/battle_city.bit"),
					doc         = URL("https://drive.google.com/open?id=1-SSevX1OZZwDJ7P5RfUtQV7fHGoBChEJ"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Super Mario",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Vukašin, Milenko, Stefan",
			members = Student[
				Student(
					name    = "Vukasin",
					surname = "Cosic",
					index   = "RA-212/2016",
					email   = "@vukasin997@hotmail.com",
					points  = 0,
				),
				Student(
					name    = "Milenko",
					surname = "Maksic",
					index   = "RA-11/2016",
					email   = "@maksicmra11@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Stefan",
					surname = "Vasovic",
					index   = "RA-59/2016",
					email   = "@vasovicstefan14@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			- Doraditi postojeću igru.
				- Animacije.
				- Skokovi.
				- Neprijatelji.
			- Koristiti skripte iz Zelde.
			""",
			resources = URL[
				URL("https://github.com/CubeLord/LPRS2-Zelda")
			],
			sequel_resources = Dict{String, Solution}(
				"Super Mario" => Solution(
					repo        = URL("https://github.com/Phaustus/Super_Mario.git"),
					youtube     = URL("https://youtu.be/JGzYIpUky0Y"),
					release_bit = URL("https://www.dropbox.com/s/wjqw4lh5y9e6kvt/Super_Mario.bit?dl=0"),
					doc         = URL("https://github.com/Phaustus/Super_Mario/blob/master/Dokumentacija2.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Tower Defense",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Teodora, Marko, Jovan",
			members = Student[
				Student(
					name    = "Teodora",
					surname = "Milacic",
					index   = "RA-198/2016",
					email   = "teodora.milacic@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Marko",
					surname = "Mihic",
					index   = "RA-149/2013",
					email   = "mihicmarkons@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Jovan",
					surname = "Sotirov",
					index   = "RA-150/2016",
					email   = "jovansotirov@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			Više level-a.
			Više feature-a.
			Bolji gameplay.
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Tower Defence" => Solution(
					repo        = URL("https://github.com/stefan9x/lprs2-TowerDefence"),
					youtube     = URL("https://www.youtube.com/watch?v=-AOyvGvH7hY"),
					release_bit = URL("https://github.com/stefan9x/lprs2-TowerDefence/blob/master/towerdefence.bit"),
					doc         = URL("https://github.com/stefan9x/lprs2-TowerDefence/blob/master/TowerDefence-dokumentacija.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Castle Defence",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
			],
			sequel_resources = Dict{String, Solution}(
				"Tower Defence" => Solution(
					repo        = URL("https://github.com/stefan9x/lprs2-TowerDefence"),
					youtube     = URL("https://www.youtube.com/watch?v=-AOyvGvH7hY"),
					release_bit = URL("https://github.com/stefan9x/lprs2-TowerDefence/blob/master/towerdefence.bit"),
					doc         = URL("https://github.com/stefan9x/lprs2-TowerDefence/blob/master/TowerDefence-dokumentacija.docx"),
				),
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "AirXonix",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Katarina, Nikolina, Nikolina",
			members = Student[
				Student(
					name    = "Nikolina",
					surname = "Gasic",
					index   = "RA-116/2016",
					email   = "nikolinagasic97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Nikolina",
					surname = "Babic",
					index   = "RA-12/2016",
					email   = "b.nikolina97@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Katarina",
					surname = "Dadic",
					index   = "RA-241/2017",
					email   = "katarinadadic9@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=QshZeuyFxvk"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Lode Runner",
		points           = 45,
		min_team_members = 2,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "Filip",
					surname = "Mihić",
					index   = "RA-19/2015",
					email   = "fikimihic@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Marko",
					surname = "Mihić",
					index   = "RA-149/2013",
					email   = "mihicmarkons@gmail.com",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = true,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Bubble Bobble",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=BuXOSBb4hQw"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Binary Land",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=wMpxE9xq2Lw"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Golf",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=SPxxfH_NOEA"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Go",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=AT0j4gZxG2w"),
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Lunar Pool",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=3Aih37orQKY"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Chuzzle",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=4vZnNZ_G6Qk"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Zuma",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=-Ou5Bco1sHM"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Save the Day",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=cgM_A1Oe0mY"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Wall Wrecker",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=xo4EABMuxCc"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Pocket Tanks",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=zguf7ZeBpA4"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Happy Glass",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=6ljDYeuOFxU"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "SimCity",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=A54blk-ojA4"),
				URL("https://github.com/stefan9x/lprs2-TowerDefence"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Crossy Road",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Tamara, Marina, Olivera",
			members = Student[
				Student(
					name    = "Tamara",
					surname = "Rudaljevic",
					index   = "RA-28/2016",
					email   = "tamararudaljevicruda@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Marina",
					surname = "Repac",
					index   = "RA-141/2016",
					email   = "marinarepac@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Olivera",
					surname = "Ivankovic",
					index   = "RA-164/2016",
					email   = "oliveraivankovic97@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=iTBiaAzbwHU"),
				URL("https://github.com/CubeLord/LPRS2-Zelda"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Tron: Slow Cycle Mastery",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "Ana, Sanja, Ognjen",
			members = Student[
				Student(
					name    = "Ana",
					surname = "Teodorovic",
					index   = "RA-191/2016",
					email   = "teodorovic.a4@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Sanja",
					surname = "Mihajlov",
					index   = "RA-218/2016",
					email   = "sanja07121997@gmail.com",
					points  = 0,
				),
				Student(
					name    = "Ognjen",
					surname = "Cadovski",
					index   = "RA-159/2016",
					email   = "ogitenis@gmail.com",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=1zv333wxZFU"),
				URL("https://github.com/suncicam/MinesweeperProjekatLPRS2"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/OgnjenX/Tron-Slow-Cycle-Mastery"),
			youtube     = URL("https://www.youtube.com/watch?v=BosFmeHZ02w"),
			release_bit = URL("https://github.com/OgnjenX/Tron-Slow-Cycle-Mastery/blob/master/Tron.bit"),
			doc         = URL("https://github.com/OgnjenX/Tron-Slow-Cycle-Mastery/blob/master/Documentation.docx"),
		),
	),
	Project(
		name             = "Bridge Building",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=xgx7eECkbk0"),
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
	Project(
		name             = "Curve Fever",
		points           = 45,
		min_team_members = 3,
		team = Team(
			pre_registration = "",
			members = Student[
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
				Student(
					name    = "",
					surname = "",
					index   = "RA-xxx/20yy",
					email   = "@",
					points  = 0,
				),
			],
		),
		spec = Spec(
			sequel       = false,
			have_support = false,
			text = """
			""",
			resources = URL[
				URL("https://www.youtube.com/watch?v=_Rvs2P6MBMU"),
				URL("https://github.com/SuvakovSrdjan/LPRS2_Asteroids_game"),
			],
			sequel_resources = Dict{String, Solution}(
			),
		),
		solution = Solution(
			repo        = URL("https://github.com/X/Y"),
			youtube     = URL(""),
			release_bit = URL(""),
			doc         = URL(""),
		),
	),
]


additional = URL[
	# Making bit file.
	"https://www.youtube.com/watch?v=RBa6gx5Mx5c",
	# VGA slow motion.
	"https://www.youtube.com/watch?v=3BJU2drrtCM",
	# How old school graphics worked.
	"https://www.youtube.com/watch?v=Tfh0ytz8S0k",
	# Training AI to play NES games.
	"https://www.youtube.com/watch?v=xOCurBYI_gY",
	"https://www.youtube.com/watch?v=qv6UVOQ0F44",
	# Presentations.
	# Fun.
]

###############################################################################
